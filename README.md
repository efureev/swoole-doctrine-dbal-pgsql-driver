...
If your Swoole runtime dispatches the `swoole.*` events, the rest is automatic: the connection goes back
to the pool on `kernel.terminate`, pools are closed on `swoole.worker_stop`. Without them —
