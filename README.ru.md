...
Если Swoole-рантайм диспатчит события `swoole.*`, остальное происходит само: соединение
возвращается в пул на `kernel.terminate`, пулы закрываются на `swoole.worker_stop`. Без них —
