...
4. Make sure the Swoole server's `hook_flags` include `SWOOLE_HOOK_PDO_PGSQL`
   (with swoole-runtime-bundle: `settings.hook_flags` in `APP_RUNTIME_OPTIONS`).
...
Under swoole-runtime-bundle the rest is automatic: the connection returns to the pool on `kernel.terminate`,
