...
## 1. HTTP handler under swoole-runtime-bundle
...
A swoole-runtime-bundle daemon is a long-lived coroutine. By default a lease lives until the coroutine ends,
i.e. forever: the connection drops out of the pool and is never reset. Release after every iteration:

```php
final class OutboxRelay extends TickingDaemon
{
...
A custom `DaemonInterface` whose `run()` returns to let the process restart: with the default
`maintenance_interval: 0` the pool holds no timer, so nothing keeps the process alive; if you enabled the
timer, call `PoolRegistry::closeAll()` (or `Timer::clearAll()`, as `TickingDaemon` does) before returning.

## 6. Console commands and migrations

- `bin/console` under `SwooleRuntimeBundle\SwooleRuntime\ConsoleApplication` — the command runs in a coroutine:
...
Under swoole-runtime-bundle two phases happen. `swoole.worker_exit` (fired under `reload_async` — on
...
- A session-level `pg_advisory_lock()` is released on release. For a lock spanning several requests use
  `pg_advisory_xact_lock()` inside a transaction or an external lock (swoole-runtime-bundle `lock.pools`).
