...
4. Убедиться, что в настройках Swoole-сервера `hook_flags` включает `SWOOLE_HOOK_PDO_PGSQL`
   (для сервера на Symfony Runtime — обычно `hook_flags` в `APP_RUNTIME_OPTIONS`).
...
Если рантайм диспатчит события `swoole.*`, остальное автоматически: соединение возвращается в пул на `kernel.terminate`,
