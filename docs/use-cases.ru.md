...
## 1. HTTP-обработчик под swoole-runtime-bundle
...
Демон swoole-runtime-bundle — долгоживущая корутина. Lease по умолчанию живёт до её завершения, то есть
...
Свой `DaemonInterface`, чей `run()` возвращается ради перезапуска процесса: при `maintenance_interval: 0`
(по умолчанию) у пула нет таймера и процесс ничто не держит; если таймер включён — перед возвратом вызовите
`PoolRegistry::closeAll()` (или `Timer::clearAll()`, как делает `TickingDaemon`).

## 6. Консольные команды и миграции

- `bin/console` под `SwooleRuntimeBundle\SwooleRuntime\ConsoleApplication` — команда выполняется в корутине:
...
Под swoole-runtime-bundle это две фазы. `swoole.worker_exit` (приходит при `reload_async` — по `max_request`,
...
- `pg_advisory_lock()` (сессионный) снимается при release. Нужен lock на несколько запросов — используйте
  `pg_advisory_xact_lock()` в транзакции или внешний lock (swoole-runtime-bundle `lock.pools`).
