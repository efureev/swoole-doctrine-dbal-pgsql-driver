...
|---|---|---|
| `kernel.terminate` | `PoolRegistry::releaseCurrentCoroutine()` — returns the connection right after the response. Under swoole-runtime-bundle it runs in the request coroutine | −1024, after everybody |
...
The `swoole.*` events are string names from swoole-runtime-bundle; there is no dependency on it, and without
it the listeners stay silent. Your own runtime → call `$registry->closeAll()` in `workerStop` yourself
