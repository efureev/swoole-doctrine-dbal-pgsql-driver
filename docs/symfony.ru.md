...
|---|---|---|
| `kernel.terminate` | `PoolRegistry::releaseCurrentCoroutine()` — отдаёт соединение в пул сразу после ответа. Под swoole-runtime-bundle выполняется в корутине запроса | −1024, после всех |
...
События `swoole.*` — строковые имена из swoole-runtime-bundle; зависимости на него нет, без него слушатели
