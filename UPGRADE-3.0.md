...
сам работает в прямом режиме, а если рантайм гоняет команды внутри `Coroutine\run()` — через пул.
...
- Если рантайм диспатчит события `swoole.worker_start`/`worker_exit`/`worker_stop`, делать ничего не нужно.
  Иначе: `PoolRegistry::closeAll()` на `workerStop`.
