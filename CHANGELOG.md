...
- Symfony-бандл с собственным деревом `swoole_doctrine_pool`: compiler pass подставляет `driverClass`,
  ...
  `swoole.worker_stop`/`worker_exit`/`before_shutdown` (закрытие) — без зависимости от swoole-runtime-bundle.
...
### Замечания по эксплуатации под swoole-runtime-bundle
...
- `console.terminate` закрывает пулы: команда под `ConsoleApplication` иначе не завершилась бы из-за таймера.
